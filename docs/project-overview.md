...
Tool 身份与某次审计目标必须分离。`tool_id` 在多次审计之间保持稳定；版本、commit、release、审计范围、标准和 Profile 绑定在 `run_id` 上。别名用于发现和归并，不得替代规范 ID。
...
截至 2026-06-12，仓库已完成 monorepo scaffold、最小 Registry commitment 合约、基础 schema、Rust Run API 和 SQLite append-only Run Event。合约记录按 `toolId -> runId` 聚合，并保存 Passport、Audit Log 与 Evidence Manifest Hash；链下 Tool Registry 尚未实现。Rust 会在创建 Run 时原子追加首个 `run_created` 事件，并将经过验证的 node、approval 和终止状态事件投影到 Run 摘要；数据库 trigger 会拒绝事件更新或删除。仓库也已完成 Stage 1：机器可读的 Audit Standard/Profile core schema、一个版本化标准，以及 `generic`、`agent_framework`、`mcp_server` 和 `cli_api_tool` 四个可离线校验的 Profile fixture。

链下 Tool Registry、别名归并、Run 与规范 Tool 的绑定、实际 Profile selector、长程审计图、事件哈希链、Evidence Manifest、Evidence Board、check-level 评分、Artifact、SSE、持久化审批记录、Passport Hash 和链上提交仍属于计划能力。现有 orchestrator 只包含 `clarify_goal` 和 `plan_audit` 两个 mock 节点，Dashboard 也只展示开发 workspace 状态。下一实现阶段是 Tool Identity and Run Binding；详细冲突和迁移顺序记录在 `docs/technical-design.md`，文档不得把这些计划能力描述为已经实现。
