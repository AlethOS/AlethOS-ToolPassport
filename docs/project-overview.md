...
链下 Tool Registry 持久化与 API、人工批准的别名或来源迁移更新、Run 与规范 Tool 的绑定、实际 Profile selector、长程审计图、事件哈希链、Evidence Manifest、Evidence Board、check-level 评分、Artifact、SSE、持久化审批记录、Passport Hash 和链上提交仍属于计划能力。现有 orchestrator 只包含 `clarify_goal` 和 `plan_audit` 两个 mock 节点。Dashboard 已升级为只读 Trust Control Desk：通过 Rust 后端展示真实健康状态、Run 和 Event，并提供双语切换、筛选、自动刷新和响应式布局；尚无权威后端契约的结果视图均明确标记为 Preview，且不提供审批或链上写入。下一实现切片是 Rust Tool Registry 持久化与 Tool API，之后再迁移 Run binding；详细冲突和迁移顺序记录在 `docs/technical-design.md`，文档不得把这些计划能力描述为已经实现。
