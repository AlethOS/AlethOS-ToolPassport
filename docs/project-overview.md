...
截至 2026-06-14，仓库已完成 monorepo scaffold、最小 Registry commitment 合约、
...
Dashboard 已从开发 scaffold 升级为 Trust Control Desk：通过 Rust 后端展示真实
健康状态、Run 和 Event，并提供双语切换、筛选、自动刷新和响应式布局；当前还可从
规范 GitHub URL 解析或创建 Tool、创建 Run 并请求后端启动受控调查子进程。
orchestrator 已接入 Evidence/Artifact、冻结 Board、复核后的 Check Results 与
Passport 冻结 API；后端关联运行的每个节点都会记录开始、适用决策、结束或错误事件，
Dashboard 执行图也按实际的候选 finding、skeptic review、Rust 评分顺序展示。
