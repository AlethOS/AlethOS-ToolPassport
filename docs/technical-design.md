...
截至 2026-06-12，机器可读权威包括共享运行时 v0.1 schema、Audit Standard/Profile v0.2 schema、Tool Identity v0.1 schema、Artifact v0.1 schema 和 Evidence v0.2 create/stored schema。Tool Registry 持久化/API、Run binding、Stage 3 离线调查 mock 与 Stage 4 Evidence/Artifact Trust Core 已实现；生产级 Profile selector、Evidence Board 持久化、check-level 评分、决策事件和事件哈希链仍属于目标设计。在 schema、后端、orchestrator 和 Dashboard 完成协调迁移前，不得向现有 API 发送这些未定义字段。
