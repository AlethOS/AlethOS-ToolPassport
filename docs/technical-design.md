...
截至 2026-06-12，机器可读权威仍是 `schemas/` 中的 v0.1 JSON Schema。本文新增的 Tool Registry、Evidence Board、check-level 评分、决策事件和事件哈希链属于目标 v0.2 设计；在 schema、后端、orchestrator 和 Dashboard 完成协调迁移前，不得向 v0.1 API 发送这些未定义字段。
...
sequenceDiagram
    ...
    U->>D: Select or submit tool and create audit run
...
`tool_id` 是由 Rust 维护的稳定、带命名空间标识。MVP 优先支持规范 GitHub 仓库标识和规范 URL；大小写、尾部斜线、重定向和常见 URL 变体必须按版本化规则规范化。名称相似不能作为自动合并依据；fork、仓库迁移、产品改名和冲突外部标识必须进入人工确认。`tool_id` 一旦被冻结 Passport 或 Attestation 引用就不得改写；来源迁移通过新增外部标识和别名处理。Tool 保存当前分类，Run 同时冻结审计时使用的 `tool_type` 与 Profile，避免后续重新分类改写历史。

目标 Audit Run 必须包含 `tool_id`、目标版本或 revision、审计目标、Standard/Profile 版本、范围、预算和状态。版本未知时必须显式记录 `unresolved`，不能把“最新版本”作为不可复查的目标值。
...
`mcp_server` 和 `cli_api_tool` 四个 Profile fixture。无法可靠识别时只能使用明确受限的
`generic` Profile。
...
`tool_fingerprint -> profile_selector` 运行逻辑仍属于 Stage 3。
...
| --- | --- | --- | --- |
| `intake_normalization` | 已绑定 `tool_id` 的 Run、用户目标、URL、本地材料引用和运行策略 | 规范化 Tool Intake、允许来源、预算和限制 | 缺少规范 Tool 或目标 revision 状态时拒绝启动；拒绝空目标、非法 URL、未授权文件和主网请求；不读取 `.env` |
...
| 状态 | 方法与路径 | 行为 |
| --- | --- | --- |
| Target | `POST /api/tools/resolve` | 按规范外部标识解析或请求创建 Tool；冲突时要求人工确认 |
| Target | `GET /api/tools` | 返回规范 Tool 列表与最近审计摘要 |
| Target | `GET /api/tools/by-id?tool_id=...` | 返回 Tool 身份、别名和关联 Run；避免 namespaced ID 中的斜线破坏路径语义 |
...
| 区域 | 当前实现 | 与目标设计的关系 |
| --- | --- | --- |
| Tool Registry / Run binding | 未实现；当前 Run 内嵌名称、类型和 URL | Contract gap；需要新增规范 Tool、别名解析和 `runs.tool_id`，并迁移创建 Run 请求 |
| Rust Run API | 已实现创建、列表、详情和事件追加 | Partial foundation；事务与状态规则可保留，但 Run 必须在进入调查图前绑定规范 `tool_id` 和目标 revision |
...
| Audit Standard / Profile | Stage 1 已完成：v0.2 core schema、Standard、四个 Profile fixtures 和离线 catalog 校验 | Compatible target inputs；Stage 3 runtime selector 尚未实现 |
...
2. **Tool identity and Run binding**：新增严格 Tool schema、规范化规则、`tools` /
   `tool_aliases` 表和 API；迁移 Run 使其绑定 `tool_id` 与目标 revision；为链上
   `runId` 编码增加跨语言 fixture。不得在本阶段实现 Tool Graph。
3. **Orchestrator investigation mock**：用 fixture 实现类型化调查回路、Board、Gap、停止条件和 Skeptic Review；Artifact 先通过受控本地 mock，随后接后端。
4. **Evidence and Artifact trust core**：扩展 schema、migration 和 API，由 Rust 分配 ID、保存内容并计算 Hash。
...
Backend 重点验证 Tool 规范化与冲突归并、Run 绑定、状态迁移、append-only 和哈希链、Evidence/Artifact Hash、路径隔离、确定性评分、JCS Hash、冻结边界、审批绑定和 mock RPC。Orchestrator 重点验证每个节点契约、Profile 分支、调查预算、无变化检测、反证检查、有限修复、恢复和人工等待。Dashboard 重点验证 Tool 聚合、实时状态、Board/Gap/Decision 映射、失败与空状态，以及人工批准参数。Contracts 继续验证最小 commitment、`toolId -> runId` 关联、event 和多版本记录。
