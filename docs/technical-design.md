...
截至 2026-06-12，机器可读权威包括共享运行时 v0.1 schema、Audit Standard/Profile v0.2 schema，以及 Tool Identity v0.1 schema。Tool Registry 持久化/API、Run binding、Evidence Board、check-level 评分、决策事件和事件哈希链仍属于目标设计；在 schema、后端、orchestrator 和 Dashboard 完成协调迁移前，不得向现有 API 发送这些未定义字段。
...
| 区域 | 当前实现 | 与目标设计的关系 |
| --- | --- | --- |
| Tool Registry / Run binding | Stage 2.1 纯契约已实现；当前 Run 仍内嵌名称、类型和 URL | Tool/Intake/Resolution schema、离线规范化与冲突 fixtures 已完成；仍需 Rust 持久化/API、人工确认更新和 `runs.tool_id` 迁移 |
...
2. **Tool identity and Run binding**：Stage 2.1 已完成严格 Tool schema、三态解析、
   规范化规则与离线 fixtures；下一步新增 `tools` / `tool_aliases`、人工确认更新和
   Rust Tool API，再迁移 Run 使其绑定 `tool_id` 与目标 revision，并为链上 `runId`
   编码增加跨语言 fixture。不得在本阶段实现 Tool Graph。
