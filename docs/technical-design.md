...
持久化/API、冻结 Evidence Board/Manifest 持久化/API，以及 Passport v0.2 与四个
承诺 Hash 的冻结持久化/API。
...
`onchain_run_id`，追加 `provenance_frozen` 事件，并把其 `event_hash` 作为
`audit_log_hash` 填入 Provenance 后在同一事务中原子保存不可修改的 Passport、
Provenance 和事件。通用 Event API 不允许伪造 `provenance_frozen`。
...
三个冻结 Hash、链上 Run ID、chain 与 contract。Passport/Provenance 和 Receipt
持久化/API 已实现；真实链上提交仍必须经过显式人工批准。
...
| Partial | `POST /api/runs/:run_id/investigate` | 在用户请求后启动或恢复仓库内受控 orchestrator 子进程；Run-bound SQLite checkpoint、持久化生命周期、进程内重复并发启动防护、退出进程回收和本地直启脚本已实现，跨进程租约和生产级进程监督仍待实现 |
...
| Implemented | `GET /api/attestation/preflight` | 从运行时配置派生并返回公开的 Sepolia chain、Signer 地址/余额、Registry 地址/代码状态和问题列表；不返回 RPC 或私钥，不签名或广播 |
...
`evidence`、`artifacts` 和 `check_results`。后续目标表包括：
...
独立 Receipt。Dashboard 还展示只含公开派生字段的 Sepolia preflight；私钥、RPC、
链 ID 校验、签名、广播和确认均由 Rust/Alloy 负责。审批中的 Registry 必须与运行时
`REGISTRY_CONTRACT` 一致。
...
6. **Deterministic checks and Passport v0.2**：已完成 Check Result
   ...
   上限、Run catalog 绑定、不可修改的 check-result API/持久化、冻结 Evidence
   Board/Manifest API/持久化，以及 JCS + SHA-256 四个承诺 Hash 与 Passport/Provenance
   冻结持久化/API。下一步进入 Stage 7。
...
Agent 不读取、修改或打印 `.env`，仓库也不提交该文件。本地启动脚本可在不回显内容的前提下解析简单 `KEY=VALUE` 配置并注入受控进程环境；显式传入的环境变量优先。运行时只能从受控进程环境接收必要配置，前端公开变量不得包含密钥。真实模型、付费 API、钱包和测试网配置均需要人工提供与授权。
