...
Standard/Profile v0.2 schema、Tool Identity v0.1 schema、Artifact v0.1 schema、
...
Stage 6 当前仅完成共享契约，Rust 确定性评分、冻结持久化和 API 仍属于目标设计。
...
| Passport 与评分 | 已发布严格 Passport v0.2 与 Check Result submission/stored 契约；Rust 评分和持久化未实现 | Partial；推理输入与 Rust-owned totals 已分离，后续实现确定性聚合与冻结 |
...
6. **Deterministic checks and Passport v0.2**：已完成 Check Result
   ...
   独立 Attestation Receipt 共享契约；下一步由 Rust 实现 check 规则、评分聚合、JCS
   Hash、`evidenceManifestHash` 和冻结持久化/API。
