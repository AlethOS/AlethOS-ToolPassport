...
2. **Tool identity and Run binding**：Stage 2.1 已完成严格 Tool schema、三态解析、
   ...
   Rust Tool API，再迁移 Run 使其绑定 `tool_id` 与目标 revision，增加可选
   `audit_directives` 字段，并为链上 `runId` 编码增加跨语言 fixture。
   不得在本阶段实现 Tool Graph。
3. **Orchestrator investigation mock**：用 fixture 实现类型化调查回路、Board、Gap、停止条件和 Skeptic Review；实现 `audit_directives` 从自由文本到结构化约束的解析与传递；Artifact 先通过受控本地 mock，随后接后端。
